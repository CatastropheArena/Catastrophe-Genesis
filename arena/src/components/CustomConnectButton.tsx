...
import { useNexusObjects } from "../lib/nexus/usePassport";
...
const CustomConnectButton: React.FC<PassportConnectButtonProps> = ({
  ...
  return (
    <Connector onChainSwitched={setCurrentChain}>
      {loading ? (
        <ConnectButton
        ...
        <ConnectButton
          ...
          actionsMenu={{
            extraItems: [
              ...
              objects?.passport && currentAccount
                ? {
                    key: "3",
                    label: "View Passport",
                    onClick: () => {
                      if (currentAccount && objects.passport?.objectId) {
                        const accountUrl = getObjectUrl(
                          "testnet",
