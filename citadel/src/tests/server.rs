...
async fn test_timestamp_updater() {
    // 创建测试集群，包含1个密钥服务器
    let mut tc = SealTestCluster::new(1, 0).await;
    ...
    // 启动时间戳更新器并获取接收通道
    let mut receiver =
        AppState::spawn_latest_checkpoint_timestamp_updater(tc.server_mut(), Some(update_interval))
            .await;
...
async fn test_rgp_updater() {
    ...
    // 启动参考燃气价格更新器并获取接收通道
    let mut receiver =
        AppState::spawn_reference_gas_price_updater(&mut tc.server_mut(), Some(update_interval))
            .await;
