...
use super::super::externals::current_epoch_time;
use super::super::signed_message::signed_request;
use super::super::{
    signed_message,
    types::{ElGamalPublicKey, ElgamalVerificationKey},
};
use super::super::keys::Certificate;
...
use super::super::keys::{check_request, create_response};
...
pub(crate) async fn get_key(
    ...
    check_request(
        ...
        .map(|ids| {
            ...
            elgamal::decrypt(
                &sk,
                &create_response(server,&ids, &pk).decryption_keys[0].encrypted_key,
            )
        })
