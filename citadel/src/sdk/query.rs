...
pub async fn query_relationship(
    ...
    for field in fields {
        ...
        // 检查是否是目标关系
        if (a == *user_id && b == *friend_id) || (a == *friend_id && b == *user_id) {
            info!("找到目标关系");
...
pub async fn query_all_relationships(
    ...
    // 用于临时存储每个用户的关系
    let mut user_relationships: std::collections::HashMap<ObjectID, Vec<RelationshipQueryResult>> = std::collections::HashMap::new();
    ...
    for field in fields {
        ...
        let relationship = RelationshipQueryResult {
            initiator,
            receiver,
        ...
        // 将关系添加到发起者和接收者的关系列表中
        user_relationships.entry(initiator)
            .or_insert_with(Vec::new)
            .push(relationship.clone());
        user_relationships.entry(receiver)
            .or_insert_with(Vec::new)
            .push(relationship);
    }
    
    // 转换为最终结果格式
    let result = user_relationships.into_iter()
        .map(|(user_id, relationships)| AllRelationshipsQueryResult {
            user_id,
            relationships,
        })
        .collect::<Vec<_>>();
    
    info!("查询完成,共获取 {} 个用户的关系数据", result.len());
    Ok(result)
} 