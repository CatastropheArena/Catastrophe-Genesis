...
pub fn male_avatar(seed: u64, mood: &Mood) -> String {
    ...
    let skin_color = to_rgb(&g.pick_one(&SKIN_COLORS));
    let hair_color =
        to_rgb(&g.pick_one(&HAIR_COLORS)).brighter_or_darker_than(&skin_color, 17.0);
    let eyes_color = to_rgb(&g.pick_one(&EYE_COLORS));
    ...
    let mouth_color = to_rgb(&g.pick_one(&male::MOUTH_COLORS))
        .brighter_or_darker_than(&skin_color, 10.0);
    let glasses_color = to_rgb(&g.pick_one(&GLASSES_COLORS));
    let clothes_color = to_rgb(&g.pick_one(&male::CLOTHES_COLORS));
    let hat_color = to_rgb(&g.pick_one(&male::HAT_COLORS));

    let mouth = match mood {
        Mood::Sad => male::mouths::SAD,
        Mood::Happy => male::mouths::HAPPY,
        Mood::Surprised => male::mouths::SURPRISED,
    };

    let _mustache = &g.pick_one(&male::MUSTACHE);
    let _glasses = &g.pick_one(&male::GLASSES);
    let _hair = &g.pick_one(&male::HAIR);
    let _hat = &g.pick_one(&HAT);
...
pub fn female_avatar(seed: u64, mood: &Mood) -> String {
    let mut g = linear_congruential_generator(seed);
    let skin_color = to_rgb(&g.pick_one(&SKIN_COLORS));
    let hair_color = to_rgb(&g.pick_one(&HAIR_COLORS)).brighter_or_darker_than(&skin_color, 17.0);
    let eyes_color = to_rgb(&g.pick_one(&EYE_COLORS));
    let eyebrows_color = hair_color
        .darker_than(&skin_color, 7.0)
        .darker_than(&hair_color, 10.0);
    let accessories_colors = to_rgb(&g.pick_one(&female::ACCESSORIES_COLORS));
    let mouth_color = to_rgb(&g.pick_one(&female::MOUTH_COLORS))
        .brighter_or_darker_than(&skin_color, 10.0);
    let glasses_color = to_rgb(&g.pick_one(&GLASSES_COLORS));
    let clothes_color = to_rgb(&g.pick_one(&female::CLOTHES_COLORS));
    let hat_color = to_rgb(&g.pick_one(&female::HAT_COLORS));

    let mouth = match mood {
        Mood::Sad => female::mouths::SAD,
        Mood::Happy => female::mouths::HAPPY,
        Mood::Surprised => female::mouths::SURPRISED,
    };

    let _accessories = &g.pick_one(&female::ACCESSORIES);
    let _glasses = &g.pick_one(&female::GLASSES);
    let _hat = &g.pick_one(&HAT);
    ...
    let mut svg = [
        ...
        mouth,
        &g.pick_a_or_b(0.25, _glasses, ""),
        &g.pick_one(&female::CLOTHES),
        &g.pick_one(&female::HAIR),
        &g.pick_a_or_b(0.05, _hat, ""),
        SVG_END,
    ]
    .join("");
