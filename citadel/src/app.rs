...
pub async fn process_data(
    ...
    let current_timestamp = std::time::SystemTime::now()
        ...
        .map_err(|e| {
            ...
            EnclaveError::GenericError(format!(
                "Failed to get current timestamp: {}, API key MD5: {}",
                e, api_key_md5
            ))
        })?
        .as_millis() as u64;
    ...
    let (twitter_name, sui_address) = match fetch_tweet_content(api_key, &user_url).await {
        Ok(result) => {
            info!(
                "Successfully retrieved data for user {}, SUI address length: {}",
                result.0,
                result.1.len()
            );
            result
        ...
        Err(e) => {
            error!(
                "Failed to fetch Twitter content: {:?}, API key MD5: {}",
                e, api_key_md5
            );
...
async fn fetch_tweet_content(
    ...
    if user_url.contains("/status/") {
        ...
        let re = Regex::new(r"x\.com/\w+/status/(\d+)").map_err(|e| {
            error!("Failed to create Twitter URL regex: {}", e);
            EnclaveError::GenericError(format!(
                "Invalid Twitter URL format: {}, API key MD5: {}",
                e, api_key_md5
            ))
        })?;
        ...
        let tweet_id = re
            ...
            .ok_or_else(|| {
                ...
                EnclaveError::GenericError(format!(
                    "Invalid Twitter URL: {}, API key MD5: {}",
                    user_url, api_key_md5
                ))
            })?;
        ...
        let response = match response_result {
            Ok(resp) => {
                info!(
                    "Received Twitter API response: status code {}",
                    resp.status()
                );
                ...
                match resp.json::<serde_json::Value>().await {
                    ...
                    Err(e) => {
                        ...
                        return Err(EnclaveError::GenericError(format!(
                            "Failed to parse Twitter API response: {}, API key MD5: {}",
                            e, api_key_md5
                        )));
                    }
                }
            ...
            Err(e) => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "Failed to send request to Twitter API: {}, API key MD5: {}",
                    e, api_key_md5
                )));
            }
        };
            ...
            .and_then(|text| text.as_str())
        {
            ...
            None => {
                error!(
                    "Unable to extract tweet content from response: {}",
                    response
                );
                return Err(EnclaveError::GenericError(format!(
                    "Unable to extract tweet content: {}, API key MD5: {}",
                    response, api_key_md5
                )));
            }
        };

        let twitter_name = match response
            .get("includes")
            ...
            .and_then(|username| username.as_str())
        {
            ...
            None => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "Unable to extract username, API key MD5: {}",
                    api_key_md5
                )));
            }
        };
        ...
        let sui_tag_pos = match tweet_text.find("#SUI") {
            ...
            None => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "No #SUI tag found in tweet, API key MD5: {}",
                    api_key_md5
                )));
            }
        };
        ...
        let sui_address_re = Regex::new(r"0x[0-9a-fA-F]{64}").map_err(|e| {
            error!("Failed to create SUI address regex: {}", e);
            EnclaveError::GenericError(format!(
                "Invalid SUI address regex: {}, API key MD5: {}",
                e, api_key_md5
            ))
        })?;
        ...
        let sui_address = match sui_address_re.find(text_before_tag) {
            ...
            None => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "No valid SUI address found before tag, API key MD5: {}",
                    api_key_md5
                )));
            }
        };
        ...
        let decoded_address = match Hex::decode(sui_address) {
            ...
            Err(e) => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "Invalid SUI address: {}, API key MD5: {}",
                    e, api_key_md5
                )));
            }
        };
        ...
        // Handle profile URL
        let re = Regex::new(r"x\.com/(\w+)(?:/)?$").map_err(|e| {
            error!("Failed to create profile URL regex: {}", e);
            EnclaveError::GenericError(format!(
                "Invalid profile URL format: {}, API key MD5: {}",
                e, api_key_md5
            ))
        })?;
        ...
        let username = re
            ...
            .ok_or_else(|| {
                ...
                EnclaveError::GenericError(format!(
                    "Invalid profile URL: {}, API key MD5: {}",
                    user_url, api_key_md5
                ))
            })?;
            ...
            .and_then(|desc| desc.as_str())
        {
            ...
            None => {
                error!(
                    "Unable to extract user description from response: {}",
                    response
                );
                return Err(EnclaveError::GenericError(format!(
                    "Unable to extract user description: {}, API key MD5: {}",
                    response, api_key_md5
                )));
            }
        };
        ...
        let sui_tag_pos = match description.find("#SUI") {
            ...
            None => {
                ...
                return Err(EnclaveError::GenericError(format!(
                    "No #SUI tag found in user description, API key MD5: {}",
                    api_key_md5
                )));
            }
        };
...
mod test {
    ...
    async fn test_verify_signature() {
        ...
        if verified.is_err() {
            // 手动签名数据，看看生成的签名是什么
            let our_signature: Ed25519Signature = kp.sign(&signing_payload);
